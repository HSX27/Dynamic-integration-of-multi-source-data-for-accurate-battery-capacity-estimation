...
# Define SVM parameters (default values, not optimized)
svm_params = {
    'kernel': 'rbf',
    'C': 1.0,
    'epsilon': 0.1
...
print(f"SVM - RMSE: {rmse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}, Training Time: {training_time:.2f}s")


# PDP and ICE plotting function
def plot_pdp_ice(model, model_name, X_train_scaled):
    feature_pairs = [('Qdlin', 'Voltage_m'), ('Qdlin', 'Temp_m'), ('Qdlin', 'Current_l')]
    selected_features = ['Voltage_m', 'Current_l', 'Temp_m', 'Qdlin']
...
plot_pdp_ice(svm_model, "SVM", X_train_scaled)
