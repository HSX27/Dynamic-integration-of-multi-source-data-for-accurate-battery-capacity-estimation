...
# Parameter ranges
pbounds = {'n_neighbors': (150, 200)}

# Perform optimization
optimizer = BayesianOptimization(f=knn_evaluate, pbounds=pbounds, random_state=42)
...
print(f"KNN - RMSE: {rmse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}, Training time: {training_time:.2f}s")

# PDP and ICE plotting functions
def plot_pdp_ice(model, model_name, X_train_scaled):
    feature_pairs = [('Qdlin', 'SOH'), ('Qdlin', 'CCCT'), ('Qdlin', 'ir')]
    selected_features = ['Qdlin', 'SOH', 'CCCT', 'ir']
    ...
    for pair in feature_pairs:
        ...
        display = PartialDependenceDisplay.from_estimator(
            model, X_train_scaled, [pair], kind='average', subsample=30,
            n_jobs=1, grid_resolution=15, random_state=42, percentiles=(0.001, 0.999)
        )
...
plot_pdp_ice(knn_model, "KNN", X_train_scaled)
