...
def lgb_evaluate(max_depth, learning_rate, n_estimators, subsample, min_child_samples, num_leaves):
    params = {
...
print(f"LightGBM - RMSE: {rmse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}, Training time: {training_time:.2f}s")

# PDP and ICE plotting functions
def plot_pdp_ice(model, model_name, X_train_scaled):
    feature_pairs = [('SOH', 'ir'), ('SOH', 'Qdlin'), ('SOH', 'chargetime')]
    selected_features = ['SOH', 'ir', 'Qdlin','chargetime']
...
plot_pdp_ice(lgb_model, "LightGBM", X_train_scaled)
