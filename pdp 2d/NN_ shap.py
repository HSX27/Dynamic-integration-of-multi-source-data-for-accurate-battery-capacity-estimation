...
X_train_scaled = pd.DataFrame(X_train_scaled, columns=features)  # Preserve feature names

# Define KerasRegressor wrapper class
class KerasRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, model):
        self.model = model
        self.is_fitted_ = False  # Add fitting status flag
...
# Create and train KerasRegressor model
keras_regressor = KerasRegressor(nn_model)
...
print(f"Keras Neural Network - RMSE: {rmse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}, Training time: {training_time:.2f}s")

# PDP and ICE plotting functions
def plot_pdp_ice(model, model_name, X_train_scaled):
    feature_pairs = [('Qdlin', 'ir'), ('Qdlin', 'chargetime'), ('Qdlin', 'Current_l')]
    selected_features = ['Qdlin', 'ir', 'chargetime', 'Current_l']
...
plot_pdp_ice(keras_regressor, "Keras Neural Network", X_train_scaled)
